...
wall (§C in docs/scaling-to-1m.md). This wraps a FAISS IVF index so the recommender
retrieves a few hundred content candidates in ~1 ms instead of scanning the catalog,
then reranks them with the exact blend.
...
class ANNIndex:
    """A trained FAISS IVF index over the catalog embeddings, in catalog-row order."""
    ...
    @classmethod
    def build(cls, emb: np.ndarray, nprobe: int = 32) -> ANNIndex | None:
        """Build an IVF index over ``emb`` (N, D), or ``None`` if ANN doesn't apply."""
        n = len(emb)
