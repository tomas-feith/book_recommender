...
| `scripts/enrich_google_books.py`| Fill missing descriptions/categories/covers on existing books via the **Google Books API** (~48% of goodbooks entries lack a description), then re-embed only the changed rows. Needs a free `GOOGLE_BOOKS_API_KEY` (the anonymous quota is a shared, usually-exhausted pool). One book per request and ~1k/day, so this is the slow trickle — prefer `enrich_bulk.py` when a dump is available. |
| `scripts/enrich_bulk.py`        | Backfill the same missing descriptions from **bulk dataset dumps** — no API, no quota. Streams the UCSD Goodreads dump (`goodreads_books.json.gz`, matched via `gb_books.csv` → `goodreads_book_id`) and/or the OpenLibrary works dump (matched on the `ol:OL…W` key) once each, fills whatever they cover, and re-embeds the changed rows. Fills ~all ~10k in one pass vs months of API calls. Pass `--goodreads` and/or `--ol-works`. |
